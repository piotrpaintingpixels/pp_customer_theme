...
function blockgrid_gallery( $output, $atts, $instance ) {
	...
	$output = '<div class="grid-x grid-padding-x grid-padding-y small-up-2 medium-up-' . intval( $atts[ 'columns' ] ) . '" >';
	...
	foreach ( $attachments as $id => $attachment ) {
		...
		$output .= '<div class="cell">'
			...
			. '<img src="' . esc_url( $img ) . '" ' . $caption . ' rel="lightbox" alt="' . esc_attr( $attachment->title ) . '"  srcset="' . esc_attr( $img_srcset ) . '" sizes="(max-width: 50em) 87vw, 680px" />'
			. '</a></div>';
	}
...
/*function foundation_image_tag_class($class) {
    $class .= ' thumbnail';
    return $class;
}*/
//add_filter('get_image_tag_class', 'foundation_image_tag_class' );